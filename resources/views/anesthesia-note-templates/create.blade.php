...
<div class="container py-4" style="max-width:800px;">
    <div class="d-flex align-items-center mb-4">
        ...
        <h2 class="mb-0"><i class="bi bi-lungs"></i> Nueva plantilla de Nota de Anestesia</h2>
    </div>
    ...
    <form method="POST" action="{{ route('anesthesiaNoteTemplates.store') }}">
        @csrf
        ...
        <div class="card mb-3">
            <div class="card-header" style="background-color:#E91E63; color:white;">
                ...
                <small class="ms-2 opacity-75">Solo las 5 secciones más reutilizables</small>
            </div>
            <div class="card-body">
                @foreach($sections as $key => $label)
                    <div class="mb-3">
                        <label class="form-label fw-semibold">{{ $label }}</label>
                        <textarea name="{{ $key }}" class="form-control" rows="4">{{ old($key) }}</textarea>
                    </div>
                @endforeach
            </div>
        </div>
        ...
        <div class="d-flex gap-2">
            <button type="submit" class="btn btn-primary"><i class="bi bi-floppy"></i> Guardar plantilla</button>
            <a href="{{ route('anesthesiaNoteTemplates.index') }}" class="btn btn-outline-secondary">Cancelar</a>
        </div>
    </form>
</div>
@endsection
