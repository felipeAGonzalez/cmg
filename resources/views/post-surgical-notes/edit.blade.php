...
<div class="container py-4" style="max-width:900px;">
    ...
    <div class="d-flex align-items-center mb-4 gap-3">
        ...
        <h2 class="mb-0">Editar Nota Postquirúrgica</h2>
    </div>
    ...
    <form method="POST" action="{{ route('postSurgicalNotes.update', $note) }}">
        @csrf
        @method('PUT')
                ...
                <div class="row g-2 mb-2">
                    <div class="col-md-4">
                        ...
                        <input type="date" name="surgery_date" class="form-control @error('surgery_date') is-invalid @enderror"
                               value="{{ old('surgery_date', $note->surgery_date?->format('Y-m-d')) }}">
                    ...
                    <div class="col-md-4">
                        ...
                        <input type="time" name="surgery_time" class="form-control @error('surgery_time') is-invalid @enderror"
                               value="{{ old('surgery_time', $note->surgery_time ? \Illuminate\Support\Str::of($note->surgery_time)->substr(0, 5) : '') }}">
                    ...
                    <div class="col-md-4">
                        ...
                        <select name="surgery_type" class="form-select">
                            <option value="">Seleccionar...</option>
                            <option value="urgencia" {{ old('surgery_type', $note->surgery_type) === 'urgencia' ? 'selected' : '' }}>Urgencia</option>
                            <option value="programada" {{ old('surgery_type', $note->surgery_type) === 'programada' ? 'selected' : '' }}>Programada</option>
                        </select>
                    </div>
                </div>
                <div class="row g-2">
                    <div class="col-md-4">
                        ...
                        <input type="text" name="surgical_time" placeholder="Ej. 2:30 hrs"
                               ...
                               value="{{ old('surgical_time', $note->surgical_time) }}">
                    ...
                    <div class="col-md-4">
                        ...
                        <input type="text" name="ischemia_time" placeholder="Ej. 45 min"
                               ...
                               value="{{ old('ischemia_time', $note->ischemia_time) }}">
        ...
        <div class="card mb-3 border-0 shadow-sm">
            <div class="card-body">
                ...
                >{{ old('preop_diagnosis', $note->preop_diagnosis) }}</textarea>
        ...
        <div class="card mb-3 border-0 shadow-sm">
            <div class="card-body">
                ...
                >{{ old('postop_diagnosis', $note->postop_diagnosis) }}</textarea>
            ...
            <div class="card-body">
                <div class="row g-3">
                    <div class="col-md-6">
                        ...
                        >{{ old('planned_surgery', $note->planned_surgery) }}</textarea>
                    ...
                    <div class="col-md-6">
                        ...
                        >{{ old('performed_surgery', $note->performed_surgery) }}</textarea>
            ...
            <div class="card-body">
                <div class="row g-3">
                    <div class="col-md-6">
                        ...
                        >{{ old('complications', $note->complications) }}</textarea>
                    ...
                    <div class="col-md-6">
                        ...
                        >{{ old('bleeding', $note->bleeding) }}</textarea>
                ...
                <div class="row g-2">
                    <div class="col-md-4">
                        <select name="textile_count" class="form-select" id="textileCountSelect"
                            ...
                            <option value="">Seleccionar...</option>
                            <option value="completo" {{ old('textile_count', $note->textile_count) === 'completo' ? 'selected' : '' }}>Completo</option>
                            <option value="incompleto" {{ old('textile_count', $note->textile_count) === 'incompleto' ? 'selected' : '' }}>Incompleto</option>
                        </select>
                    </div>
                    <div class="col-md-8" id="textileDetailWrapper"
                         style="{{ old('textile_count', $note->textile_count) === 'incompleto' ? '' : 'display:none;' }}">
                        <input type="text" name="textile_count_detail" class="form-control"
                               placeholder="Especificar detalle del recuento incompleto"
                               value="{{ old('textile_count_detail', $note->textile_count_detail) }}">
                    </div>
                </div>
            </div>
        </div>
        ...
        <div class="card mb-3 border-0 shadow-sm">
            <div class="card-body">
                ...
                >{{ old('patient_status_at_exit', $note->patient_status_at_exit) }}</textarea>
                ...
                >{{ old('prognosis', $note->prognosis) }}</textarea>
        ...
        <div class="card mb-3 border-0 shadow-sm">
            ...
            <div class="card-body">
                ...
                {{-- Cirujano --}}
                @php
                    $surgeonIsOther = is_null($note->surgeon_user_id) && !empty($note->surgeon_other_name);
                    $assistantIsOther = is_null($note->assistant_user_id) && !empty($note->assistant_other_name);
                    $anesthIsOther = is_null($note->anesthesiologist_user_id) && !empty($note->anesthesiologist_other_name);
                @endphp
                    ...
                    <div class="col-md-6">
                        ...
                        <select name="surgeon_user_id" class="form-select"
                            ...
                            @foreach($doctors as $doc)
                                <option value="{{ $doc->id }}"
                                        {{ old('surgeon_user_id', $surgeonIsOther ? null : $note->surgeon_user_id) == $doc->id ? 'selected' : '' }}>
                                    Dr(a). {{ $doc->name }} {{ $doc->last_name_one }}
                                </option>
                            @endforeach
                            <option value="other"
                                    {{ old('surgeon_user_id', $surgeonIsOther ? 'other' : null) === 'other' ? 'selected' : '' }}>
                                Otro (especificar)
                            </option>
                        </select>
                    </div>
                    <div class="col-md-6" id="surgeon_other_name_wrapper"
                         style="{{ old('surgeon_user_id', $surgeonIsOther ? 'other' : null) === 'other' ? '' : 'display:none;' }}">
                        ...
                        <input type="text" name="surgeon_other_name" class="form-control"
                               value="{{ old('surgeon_other_name', $note->surgeon_other_name) }}"
                ...
                <div class="row g-2 mb-3">
                    <div class="col-md-6">
                        ...
                        <select name="assistant_user_id" class="form-select"
                            ...
                            @foreach($doctors as $doc)
                                <option value="{{ $doc->id }}"
                                        {{ old('assistant_user_id', $assistantIsOther ? null : $note->assistant_user_id) == $doc->id ? 'selected' : '' }}>
                                    Dr(a). {{ $doc->name }} {{ $doc->last_name_one }}
                                </option>
                            @endforeach
                            <option value="other"
                                    {{ old('assistant_user_id', $assistantIsOther ? 'other' : null) === 'other' ? 'selected' : '' }}>
                                Otro (especificar)
                            </option>
                        </select>
                    </div>
                    <div class="col-md-6" id="assistant_other_name_wrapper"
                         style="{{ old('assistant_user_id', $assistantIsOther ? 'other' : null) === 'other' ? '' : 'display:none;' }}">
                        <label class="form-label small">Nombre (si no es usuario del sistema)</label>
                        <input type="text" name="assistant_other_name" class="form-control"
                               value="{{ old('assistant_other_name', $note->assistant_other_name) }}"
                               placeholder="Nombre completo">
                    </div>
                </div>
                ...
                <div class="row g-2">
                    <div class="col-md-6">
                        ...
                        <select name="anesthesiologist_user_id" class="form-select"
                            ...
                            @foreach($doctors as $doc)
                                <option value="{{ $doc->id }}"
                                        {{ old('anesthesiologist_user_id', $anesthIsOther ? null : $note->anesthesiologist_user_id) == $doc->id ? 'selected' : '' }}>
                                    Dr(a). {{ $doc->name }} {{ $doc->last_name_one }}
                                </option>
                            @endforeach
                            <option value="other"
                                    {{ old('anesthesiologist_user_id', $anesthIsOther ? 'other' : null) === 'other' ? 'selected' : '' }}>
                                Otro (especificar)
                            </option>
                        </select>
                    </div>
                    <div class="col-md-6" id="anesthesiologist_other_name_wrapper"
                         style="{{ old('anesthesiologist_user_id', $anesthIsOther ? 'other' : null) === 'other' ? '' : 'display:none;' }}">
                        <label class="form-label small">Nombre (si no es usuario del sistema)</label>
                        <input type="text" name="anesthesiologist_other_name" class="form-control"
                               value="{{ old('anesthesiologist_other_name', $note->anesthesiologist_other_name) }}"
                               placeholder="Nombre completo">
                    </div>
                </div>
            </div>
        ...
        <div class="card mb-3 border-0 shadow-sm">
            <div class="card-body">
                ...
                >{{ old('surgical_technique', $note->surgical_technique) }}</textarea>
                @error('surgical_technique')<div class="invalid-feedback">{{ $message }}</div>@enderror
            </div>
        </div>

        <div class="d-flex justify-content-between mb-5">
            <a href="{{ route('postSurgicalNotes.show', $note) }}" class="btn btn-secondary">Cancelar</a>
