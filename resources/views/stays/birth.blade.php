...
<div class="container py-4" style="max-width:720px;">
    ...
    <div class="d-flex align-items-center gap-2 mb-4">
        <a href="{{ route('stays.show', $room) }}" class="btn btn-outline-secondary btn-sm">
            <i class="bi bi-arrow-left"></i>
        </a>
        <h4 class="mb-0 fw-bold">
            <i class="bi bi-balloon-heart me-2"></i>Registrar nacimiento — Cuarto {{ $room->number }}
        </h4>
    </div>

    <div class="alert alert-info border-0 shadow-sm mb-4">
        <i class="bi bi-info-circle me-1"></i>
        El recién nacido será ingresado como paciente en el mismo cuarto que la madre
        (<strong>{{ $mother->patient->fullName() }}</strong>). Tendrá su propio expediente,
        documentos y se podrá dar de alta de forma independiente.
    </div>
    ...
    <form method="POST" action="{{ route('stays.storeBirth', $room) }}">
        @csrf

        {{-- Datos del recién nacido --}}
        <div class="card border-0 shadow-sm p-4 mb-4">
            <h6 class="fw-bold mb-3 text-primary border-bottom pb-2">
                <i class="bi bi-person me-1"></i>Datos del recién nacido
            </h6>
            ...
            <div class="row g-3">
                ...
                <div class="col-md-6">
                    ...
                    <input type="text" id="last_name_one" name="last_name_one"
                           ...
                           value="{{ old('last_name_one', $mother->patient->last_name_one) }}" maxlength="100" required>
                    @error('last_name_one')
                        <div class="invalid-feedback">{{ $message }}</div>
                    @enderror
                </div>
                ...
                <div class="col-md-6">
                    ...
                    <input type="date" id="birth_date" name="birth_date"
                           ...
                           value="{{ old('birth_date', now()->format('Y-m-d')) }}" required>
                    @error('birth_date')
                        <div class="invalid-feedback">{{ $message }}</div>
                    @enderror
                </div>
        ...
        <div class="card border-0 shadow-sm p-4 mb-4">
            ...
            <div class="mb-3">
                <label for="diagnosis" class="form-label fw-semibold">Diagnóstico / Notas</label>
                <textarea id="diagnosis" name="diagnosis" rows="4" maxlength="2000"
                          class="form-control @error('diagnosis') is-invalid @enderror"
                          required>{{ old('diagnosis', 'Recién nacido') }}</textarea>
                @error('diagnosis')
                    <div class="invalid-feedback">{{ $message }}</div>
                @enderror
            </div>
            ...
            <div class="mb-3">
                <label for="admission_date" class="form-label fw-semibold">Fecha y hora de nacimiento / ingreso</label>
        ...
        <div class="d-flex gap-2">
            ...
            <a href="{{ route('stays.show', $room) }}" class="btn btn-outline-secondary">Cancelar</a>
        </div>
    </form>
</div>
@endsection
