...
<div class="container py-4">
    ...
    <div class="d-flex justify-content-between align-items-center mb-4">
        <div>
            <h2><i class="bi bi-clipboard-pulse"></i> Editar Hoja de Triage</h2>
    ...
    <form method="POST" action="{{ route('triage.update', $triage) }}" id="triage-form">
        @csrf
        @method('PUT')

        @include('triage._form', ['triage' => $triage, 'now' => now()])

        <div class="d-flex justify-content-between mb-5">
            <a href="{{ route('triage.show', $triage) }}" class="btn btn-secondary">
                <i class="bi bi-x"></i> Cancelar
            </a>
            <button type="submit" class="btn btn-primary">
                <i class="bi bi-check-circle"></i> Actualizar hoja de triage
            </button>
        </div>
    </form>
</div>
